...
public class EmployeeDAO {
    ...
    public static void insertEmployee(String name, String dept, double salary) {
        try (Connection con = DBUtil.getConnection()) {
            ...
            String sql = "INSERT INTO employee_tb(name, department, salary) VALUES (?,?,?)";
    ...
    public static void getAllEmployees() {
        try (Connection con = DBUtil.getConnection()) {
            ...
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();System.out.println("ID\tName\tDepartment\tSalary");
            while (rs.next()) {
                System.out.println(
                        rs.getInt("id") + "\t" +
                        rs.getString("name") + "\t" +
    ...
    public static void getEmployeeById(int id) {
        try (Connection con = DBUtil.getConnection()) {
            ...
            String sql = "SELECT * FROM employee_tb WHERE id = ?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
    ...
    public static void updateEmployee(int id, String name, String dept, double salary) {
        try (Connection con = DBUtil.getConnection()) {
            ...
            String sql = "UPDATE employee_tb SET name=?, department=?, salary=? WHERE id=?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, name);
            ps.setString(2, dept);
            ps.setDouble(3, salary);
    ...
    public static void deleteEmployee(int id) {
        try (Connection con = DBUtil.getConnection()) {
            ...
            String sql = "DELETE FROM employee_tb WHERE id=?";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
